...
foreCasticApp.controller('WeatherReportCtrl', ['$rootScope', '$scope', '$http', function($rootScope, $scope, $http) {
  ...
  $scope.parseForecastData = function( data, n ) {
    var timeStamp = new Date( data.getTime() * 1000 ),    // forecastIO returns secs, needs millisecs
      ...
      // console.log( data );
      console.log( 'recordHi' );
      console.log( data[n] );
      // console.log( data.recordHi );
      // console.log( '' );
      // console.log( '' );
      // console.log( '' );
      // Real deal will require much data
    var randomLowDate = timeStamp.getFullYear() - Math.floor(Math.random() * 60),
  ...
  $scope.createForecast = function( location ) {
    ...
    var ForecastIOConditions = require(['assets/js/forecast.io'], function(ForecastIOConditions ) {
      $scope.forecastIOConditions = new ForecastIOConditions({
        PROXY_SCRIPT: 'proxy.php',
        API_KEY: 'f718d5c4193851fda1f4441b7fd1ad88'
      });
      console.log( $scope.forecastIOConditions );
//       $scope.forecastIOConditions.requestData( location, function(conditions) {
       $scope.forecastIOConditions.requestAllLocData( location, function(conditions) {
    ...
    var ForecastIO = require(['assets/js/forecast.io'], function(ForecastIO ) {
      ...
      $scope.forecastIO.getCurrentConditions( location, function(conditions) {
        ...
        for (var i = 0; i < conditions.length; i++) {
          // console.log( 'current conditions[' + i + ']' );
          // console.log( conditions[i] );
          var conditions = $scope.parseForecastData( conditions[i], i );
          console.log( 'conditions: ');
          console.log( conditions );
          $scope.data.unshift( conditions);
        }
        console.log( '$scope.data (current)' );
        console.log( $scope.data );
        console.log( $scope.data[0].recordHi );
        $scope.returnForecast( $scope.data );
      ...
      $scope.forecastIO.getForecastWeek( location, function(conditions) {
        ...
        for (var i = 1; i <= forecastRange; i++) {
          // console.log( 'upcoming conditions[' + i + ']' );
          // console.log( conditions[i] );
          $scope.data.push( $scope.parseForecastData( conditions[i], i ) );
        }
        // console.log( '$scope.data (upcoming)' );
        // console.log( $scope.data );
        $scope.returnForecast( $scope.data );
